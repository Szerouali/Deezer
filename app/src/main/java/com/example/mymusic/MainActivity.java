...
public class MainActivity extends AppCompatActivity implements IMusicSelected {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        //Si mode téléphone, ajoute le Layout phone (activity_main) et le searchFragment
        if (findViewById(R.id.frameLayout) != null) {
            searchFragment = new SearchFragment();
            musicFragment = new MusicFragment();
            getSupportFragmentManager().beginTransaction().add(R.id.frameLayout, searchFragment).commit();

            //sinon ajoute les fragments du layout tablette (activity_main Large)
        } else {
    ...
    public void onBackPressed() {
        //Si condition remplie, cache musicFragment & montre searchFragment
        if (musicFragment.isAdded() && findViewById(R.id.frameLayout) != null && musicFragment.isVisible()) {
            ...
            //sinon, fait un super backPress, aka ferme l'appli
            super.onBackPressed();
        }
    }
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        if (item.getItemId() == R.id.action_favorites) {
            ...
            //Si pas en mode tablette, retour en arrière, sinon, ferme l'appli
            if (findViewById(R.id.frameLayout) != null) {
                onBackPressed();
            }
        }
    ...
    public void onMusicSelected(Music music) {
        if (findViewById(R.id.frameLayout) != null) {
            ...
            if (!musicFragment.isAdded()) {
                getSupportFragmentManager().beginTransaction().hide(searchFragment).add(R.id.frameLayout, musicFragment).commit();
            } else {
                getSupportFragmentManager().beginTransaction().hide(searchFragment).show(musicFragment).commit();
            }
            //Appel de la fonction de sélection
            musicFragment.select(music);
        } else {
            //affiche le musicfragment
